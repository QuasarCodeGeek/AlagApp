<?php
    ...
    $hanap = $_GET["search"];
    $search = "SELECT * FROM alagapp_db.tbl_notedetail WHERE 
        userid LIKE '%".$hanap."%' OR 
        petid LIKE '%".$hanap."%' OR
        ndescription LIKE '%".$hanap."%' OR
        ndate LIKE '%".$hanap."%' OR
        nstatus LIKE '%".$hanap."%'";
...
<?php
    if($research->rowCount()>0 && $entry1->rowCount()>0 && $entry2->rowCount()>0){
        ...
        "<div class='card m-1 p-1 col-flex' style='width: 12rem;'>
            <div class='card-body'>
                <button type='button' class='btn btn-outline-success w-100' onClick='prescriptionEdit(".$rowsearch['nid'].")' data-bs-toggle='modal' data-bs-target='#boxModal')><h6 class='card-title'>".$rowentry2['petname']."</h6></button><br>
                ...
                <p class='card-text float-start p-1 rounded w-100' style='font-size: 12px; background-color: #E8F5E9;'>Description:<br>".$rowsearch['ndescription']."</p><br>
                <label class='card-text float-start' style='font-size: 12px;'>Date Issued:</label><strong class='card-text float-end' style='font-size: 12px;'>".$rowsearch['ndate']."</strong><br>
                <label class='card-text float-start' style='font-size: 12px;'>Status:</label><strong class='card-text float-end' style='font-size: 12px;'>".$rowsearch['nstatus']."</strong>    
            </div>
        </div>";
        $i++;
        }
