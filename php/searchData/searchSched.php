<?php
    ...
    $search = "SELECT * FROM alagapp_db.tbl_scheduler";
...
<?php
    if($research->rowCount()>0 && $entry1->rowCount()>0 && $entry2->rowCount()>0){
        ...
        "<div class='card m-1 p-1 col-flex' style='width: 12rem;'>
            <div class='card-body'>
                <button type='button' class='btn btn-success w-100' onClick='scheduleEdit(".$rowsearch['qid'].")' data-bs-toggle='modal' data-bs-target='#boxModal')><h6 class='card-title'>".$rowentry2['petname']."</h6></button><br><br>
